...
Shader "BruteForceURP/GrassLiteURP"
{
	Properties
	{
		[Header(Tint Colors)]
		...
		SubShader
		{
			...
			half4 frag(g2f i) : SV_Target
			{
				...
				col.xyz *= saturate(lerp(_SelfShadowColor, 1, pow(abs(i.color.x), 1.1)) + (_GrassShading - noise.x*dis.x * 2) + (1 - NoGrass.r) - noise.x*dis.x * 2);
				col.xyz *= _Color;
